...
s=rnorm(1000,15,10) # n, mean and sd
...
print(meanS)
```
 #DG notes: we can also direclty use function like mean to calculate mean directly as well. Same goes for variance. We are using loops and formula's here for learning purpose.
...
#now we initialize a variable for the sum of squares
sumSQ = 0.0
#Here we initialize the loop
for(i in c(1:1000)){
...
1) Use `workflowr` to set up the file structure for this lab. **(2.5 pts)**
In another .R file
...
```{r Calculating and comparing means and variances distribution }
# set seed
set.seed(100)

#lets set up initial vectors to store values later on
means <- rep(0,100) #create a vector with all 0 (100 zero's)
#means
variances <- rep(0,100)  #create a vector with all 0 (100 zero's)
b <- rep(1,10) # create a vector with all 1 (10 1's) because each sample below will have 10 random values. 

# loop to sample and store 100 means and 100 variances
for (i in 1:100) { # loop 100 times
  #sample 10 values from a normal distribution with mean = 5 and var = 9 or sd = sqrt(9)
  samples <- rnorm(n = 10, mean = 5, sd = sqrt(9))
  
  #calculate the mean using vector multiplication
  sum_samples <-  c(t(samples) %*% b) # multiply the vector by another vector of all 1's to get sum of that vector. Now using c() to convert matrix into scalar form. Otherwise warning of "Use c() or as.vector() instead" will pop up 
  mean_samples <- sum_samples/length(samples)
  means[i] <- mean_samples # to store into above vector
  
  #calculating variances using centering and sum of square
  #first centering and then calculating sum of squares for samples
  samples_centered <-samples - mean_samples
  sumSQ_samples <- t(samples_centered) %*% samples_centered # we can calculate the sum of squares using a loop as in example above or by vector multiplication with its transpose like here
  var_samples <- sumSQ_samples/(length(samples) -1 ) # Variance of the sample (unbiased estimator)
  variances[i] <- var_samples
  
}

# now lets visualize the distributions
par(mfrow = c(1, 2)) # easier to compare and observe 2 plots side-by-side

# Histogram for means
hist(means, main = "Distribution of Sample Means Lab1",
     xlab = "Sample Mean", col = "skyblue", border = "black")

# Histogram for variances
hist(variances, main = "Distribution of Sample Variances Lab1", 
     xlab = "Sample Variance", col = "lightgreen", border = "black")
```
...
# Answer:
I observed differences in center, spread and shape. 
- Center : In sample means histogram, we can see means are centered around 5 (which is population mean) whereas in sample variance histogram, the variances are centered around 9-10 (which is close to population variance). 
- Spread: Sample means are less spread or have smaller variability(around 2 to 8) whereas sample variances are widely spread (0 to 30).
- Shape: sample means are almost bell shaped i.e. normal distribution whereas sample variance are skewed (right-skewed distribution) which makes sense since it is always positive as it is a squared measure.

These suggest especially at smaller sample sizes, sample means converge faster to true mean than variances.
