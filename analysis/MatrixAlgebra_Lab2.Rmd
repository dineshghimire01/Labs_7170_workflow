...
Represent: sum of the response variable (y) for each treatment 1, 2 and 3 respectively as defined in columns of X.
...
Answer: 
- XtX represents the structure of the design matrix X showing in principal diagonal, how many observations are in each treatment or group.
...
For eg. if i had treatment (like here) and also location factor, then other non-diagonal element values will suggest how many y values have both location and treatment. 
...
These coefficients (beta_hat1,2,3) quantify the relationship between the predictor variables (X) and the response variable (y)
