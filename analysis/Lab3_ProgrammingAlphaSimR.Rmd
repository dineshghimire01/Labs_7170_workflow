...
# here::i_am is anchoring towards our project
# finding a base
```
...
## DG notes: source reads whole separate file to accept its input from the named file or URL or connection.
```{r document packages}
...
nFounders <- 100 # starting population (eg. landraces or wild plants or animals)
...
#DG notes: from AlphaSimR html file, there are generally 4 steps:
...
# 2.Setting Simulation Parameters: New global simulation parameters from founder haplotypes
# i. first line initializes an object containing the simulation parameters
SP <- AlphaSimR::SimParam$new(founderHaps)

# Additive trait architecture
# By default, the genetic variance will be 1
# ii. The next line of code defines the quantitative trait used for selection
SP$addTraitA(nQtlPerChr=nQTLperChr) 
### DG notes, A at the end is for additive effects only. for additive and dominance, there is AD and for additive and epistatis there is AE and so on.

# iii. # Create a new population of founders (now diploid created) by pairing random haplotypes to form diploid.
### in this steop, the founder haplotypes are paired together randomly to form diploid individuals, meaning each individual will have two haplotypes (from two parents)
founders <- AlphaSimR::newPop(founderHaps, simParam=SP) # takes two random haplotype and makes diploir 
str(founders)

#id's are character and iid are integers
```
...
# here we have founders population. so we can access individuals from popn like we get numbers from a vector.
test <- founders[c(2, 3, 5, 8, 13)]
testID <- test@id # get id's
firstID <- testID[order(testID)][1] #order those id's
...
# does 3 generations of single seed descent
```
...
fracHet <- sum(qtl == 1) / (nFounders * nChr * nQTLperChr) # so here (nFounders * nChr * nQTLperChr) is total number of genotypes and when we set qtl==1, we are taking all heterozygous loci for fraction of Het.
...
# here with 1/ / 2^nGenSelf, we are assuming that after nGenSelf generation of selfing, the expected fraction of het loci will decrease exponentially. 

```


> What was wrong with my reasoning about the Expected fraction heterozygous?
not all loci are heterozygous to begin with (but we set qtl ==1, so that means we only selected het loci)
...
```{r Pick random pair}
randomPair <- inbredPop[sample(nFounders, 2)] # each time separate picked
```
...
```{r Make hybrids}
# here we first create a matrix with two columns and byrow = t (filled by rows, 1, 2, 1, 2..., maternal, paternal...). The data includes nF1's number of 1and 2 alternateively. 
# In this matrix, each row represents a cross between parent1(1) and parent 2(2).
crossPlan <- matrix(rep(1:2, nF1s), ncol=2, byrow=T) # matrix(data = , nrow =, ncol =, byrow = , dimnames =)

#now makecross function perform actual crosses between pairs specified in the crossPlan above. so we have parent population (randomPair) and the crossing plan (crossPlan). thus as a result nf1's are created. 200 here
f1_pop <- AlphaSimR::makeCross(randomPair, crossPlan) # to make crosses, use this function makeCross and for that we need popn and 
head(crossPlan)
# here using crossPlan (which has 200 F1s in row with 1,2... meaning parent 1, parent2) and randomPair
```
...
```{r Phenotype F1s}
f1_pop <- AlphaSimR::setPheno(f1_pop, varE=1) # error variance set as 1. initially in founders, genetic variance is 1. so Heritability is 1/(1+1) =0.5
...
```
# due to sampling error, phenotypic variance is not exactly equarl to geno + error = 0.044 +1 + 1.044 (thereotically and if we run for infinite times) but we got 0.941 due to sampling error. the values can change...cz random
...
- makes crosses using random mating and crossPlan.
- generate F1's
- get phenotypes of parent 
- get phenotypes of progeny
- make a scatter plot
- run linear model for regression coefficient and interpret it
...
nSitesPerChr <- 1000 # Number of segregating sites _per chromosome_
nQTLperChr <- 10 # we can vary this parameter to get oligo- versus poly- genic traits
nF1s <- 200 # We are going to make F1s by crossing two parents to illustrate regression to the mean.
...
# iii. # Create a new population of founders (now diploid created) by pairing random haplotypes to form diploid.
### in this step, the founder haplotypes are paired together randomly to form diploid individuals, meaning each individual will have two haplotypes (from two parents)
founders <- AlphaSimR::newPop(founderHaps, simParam=SP) # takes two random haplotype and makes diploir 
str(founders)
```
...
```{r Get founders phenotype}

# use setpheno package in AlphaSimR to get phenotypes and set varE to 1.
founders_pheno <- AlphaSimR::setPheno(founders, varE=1)
pheno_founders <- founders_pheno@pheno
pheno_founders
```
...
```{r crossing two parents to make F1 progeny}
# First lets create a crossing plan in a matrix form as above
# here i will create a matrix by randomly sampling 200(f1's needed) parents and since we have only 100 parents or founders, we set replace = TRUE. then we arrange that data into two columns and set it true by row.
crossPlan_2 <- matrix(data = sample(x=nFounders, size = 200*2, replace = T), ncol = 2, byrow = T) # size 200*2 for 200 rows of 2 parents each ( thus total 400 parents selected from 100 founders meaning replacement true here)
head(crossPlan_2)

# Second, since we already have randomly selected parents in crossPlan, we can cross them to create F1's.
f1_popn <- AlphaSimR::makeCross(founders, crossPlan_2)
f1_popn
```
...
```{r phenotype f1 progenies}
f1_popn <-AlphaSimR::setPheno(f1_popn, varE = 1)
pheno_f1 <- f1_popn@pheno
pheno_f1 # since there are 200 F1's, it should be 200 phenotypic values.
```

5. Use each row of the `crossPlan` to find the two parents and calculate their phenotypic mean  

```{r}
# parent means and mid-parent is same here

#since in each row, I have two randomly selected parents, i can use apply function to calculate parent means for each row. 
parent_means <- apply(X= crossPlan_2, MARGIN = 1, function(row){ #margin =1 means row-wise operation
  mean(pheno_founders[row])
} )
parent_means # print the vector of parent means which should be 200 considering 200 pairs of parents needed to cross for creating 200 hybrid's.
```
...
- Here, Each black dot represents a pair of parent and progeny phenotype means.

- The blue line is the regression line fitted to the data. It represents the predicted values between mean phenotype of parents and progeny phenotypes

- The shaded region around the dark blue line is for condifence interval which shows the range where the true regression line can fall into.

```{r}
# Combine parent means and progeny phenotypes to make a dataset
regression_data <- data.frame(ParentMean = parent_means, Trait1 = pheno_f1)
head(regression_data)

# Plot regression to the mean
library(ggplot2)
ggplot(regression_data, aes(x = ParentMean, y = Trait1)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  labs(title = "Regression to the Mean _dg663", x = "Mean Phenotype of Parents", y = "Phenotype of Progeny") +
  theme_minimal()
...
#fit a linear model as observed in scatter plot above

lm <- lm(Trait1 ~ParentMean, data = regression_data)
coef(lm) #second one here is the regression coefficient where first one is intercept or mean
coef(lm)[2]
#0.4702 #when i calculated here
```

> What is the regression coefficient?  

- Here we got the moderate regression value to the means meaning "For every 1 unit increase in mean phenotype of parents, only 0.47 units increase in progeny mean phenotype". ie. 47% of variation in phenotype of progeny is explained by the mean phenotypes of their parent and so remaining 53% by other facturs like environment, non additive effects or measurement errors...

- BUT while knitting, the values are changing every time, so above numbers in interpretation changes accordingly. I couldn't get same values even by setting seed.  Please let me know if there is a way to do that. 



> How much closer to the mean are offspring, on average, compared to their parents?  QUESTION CANCELED 
